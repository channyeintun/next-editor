...
describe("uploadLesson", () => {
  it("uploads only the .ne file for a recording with no media, then creates the draft", async () => {
    mockedPut.mockImplementationOnce((_url, body, config) => {
      ...
      return Promise.resolve({ data: { path: "lessons/lesson-1/lesson-1.ne" } });
    });
    mockedPost.mockResolvedValueOnce({ data: { id: "lesson-1", slug: "test-lesson-1" } });
  ...
  it("uploads audio and camera as separate sibling files when present", async () => {
    ...
    mockedPut.mockImplementation((url) => {
      if (typeof url === "string" && url.includes(".ne")) {
        return Promise.resolve({ data: { path: "lessons/lesson-2/lesson-2.ne" } });
      }
      if (typeof url === "string" && url.endsWith(".ogg")) {
        return Promise.resolve({ data: { path: "lessons/lesson-2/lesson-2.ogg" } });
      }
      return Promise.resolve({ data: { path: "lessons/lesson-2/lesson-2.webm" } });
    });
    mockedPost.mockResolvedValueOnce({ data: { id: "lesson-2", slug: "test-lesson-2" } });
    ...
    expect(mockedPost).toHaveBeenCalledWith(
      "/lessons",
      expect.objectContaining({ tags: ["intro", "basics"], description: "desc" }),
    );
  });
  ...
  it("propagates an upload failure without creating the draft", async () => {
    ...
    await expect(
      uploadLesson(
        "lesson-3",
        { recording: createRecording(), title: "Fails", description: "", tags: [] },
        () => {},
      ),
