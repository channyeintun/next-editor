...
runtime:
  ...
  dockStartsCollapsed: false # optional; see the runner dock, below
  fixture:
    latencyMs: 1200
...
- **The console carries the register file.** After a run the runner appends the
  registers the program changed, four to a line
...
| `runtime.collapseDock` | —                                                                     | Collapses the runner dock, giving the board back to the code once the output has been read. `runtime.run` re-opens it, because that is where the output is about to land, so a script that wants the dock shut _before_ the first run declares `dockStartsCollapsed: true` on its `runtime` block instead — pinned before the clock starts, so the dock is shut in frame one rather than visibly closing a few hundred milliseconds in. Use this action for the stretches after a run. No-op if already collapsed. Any runtime kind except `none`.                                                                                                                                                                                                                          |
