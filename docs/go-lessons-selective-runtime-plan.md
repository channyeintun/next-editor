...
| Recorded Go lesson playback                     | Recorded source and console state  | Never calls the Playground API automatically      |
...
6. Support one editable `main.go` program per lesson in v1.
...
- multiple source files, packages, modules, or workspaces;
...
The Playground supports tests/examples and multi-file txtar input. A later product phase may add
visible tests or a small multi-file pure-Go workspace after usage proves the need. Full project
execution, external services, terminals, and servers remain a separate future runtime-adapter
project.
...
2. Next Editor loads a `main.go` starter locally without booting WebContainer or making an API call.
3. Monaco provides Go syntax highlighting and normal editor behavior.
4. The author records editing and explanation as usual.
5. Clicking **Run** sends the current `main.go` source to the first-party Worker endpoint.
...
1. A viewer can load and play the recorded Go lesson without authentication and without calling the
   Playground API.
2. Recorded source and console results replay from the lesson timeline.
...
5. An authenticated Run compiles the current source as a new request; it does not reuse the author's
   original source.
...
- upstream unavailable or rate-limited: preserve the source and show a retryable service message;
...
flowchart LR
    ...
    Select -->|go| Local[Monaco + in-memory main.go]
    Local --> Intent{Explicit Run?}
    Intent -->|no / playback| Recorded[Recorded source + console state]
    Intent -->|yes| Route[Main Worker /api/go-playground/run]
    Route --> Auth[Application auth + policy checks]
    Auth --> Cache{Source-result cache}
...
port, preview, or teardown API. The current `main.go` text remains in the existing workspace store.
...
3. accept JSON containing only `source`;
4. enforce a 64 KiB UTF-8 source limit before proxying;
5. reject empty source and requests outside the Go lesson contract where cheaply detectable;
6. hash the source plus request flags for cache lookup;
7. post URL-encoded `body`, `version=2`, and `withVet=true` upstream;
8. send a unique Next Editor user agent and a 20-second upstream timeout;
9. validate and normalize the upstream JSON response;
10. cache deterministic successful and compiler-error responses for at most one hour;
11. return structured `429`, `502`, and `504` errors for policy, upstream, and timeout failures;
12. record aggregate latency/status telemetry without logging source or output.
...
4. Record observed CORS, redirect, timeout, response, and error behavior without depending on
   undocumented fields.
5. Approve the v1 pure-lesson restrictions and author-facing disclosure that source is sent to the
...
5. Add a lazily loaded starter containing one `main.go` with a bounded stdout example.
...
- A fresh Go lesson opens and edits `main.go` with Go syntax highlighting.
...
2. Add source-size, timeout, response-shape, and upstream-status handling.
...
7. Ensure source, output, and diagnostics never enter application logs or telemetry.
8. Add a dependency-boundary test preventing imports from `remote-runtime/`.

Acceptance:

- Valid source returns normalized output.
- Invalid source returns normalized compiler diagnostics, not an HTTP failure.
...
2. Read the current `main.go` source at click time.
...
8. Preserve unsaved source when authentication, rate limit, timeout, or upstream failure occurs.

Acceptance:

- Editing and running a pure `main.go` program works from the lesson editor.
...
| Worker auth/policy  | auth, kill switch, size limit, rate limit, cache, timeout                   |
...
- Treat source as user content and validate size before proxying.
- Tell users that Run sends the current source to the Go Playground service.
- Never send application secrets, cookies, tokens, workspace metadata, lesson metadata, or user
  identity upstream.
- Never log source, output, compiler diagnostics, or panic text.
...
| Source is accidentally logged                   | Privacy exposure                         | Structured metrics only and tests that assert redaction                                   |
...
- An authenticated user can explicitly Run the current `main.go` and see compiler, vet, stdout,
  stderr, panic, and exit results.
- Public playback replays recorded source and console state without a live API call.
