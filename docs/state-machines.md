...
const guards = {
  ...
  canPlay: ({ context }) => 
    ...
    context.recording.frames?.length > 0,

  // Has recording loaded
  hasRecording: ({ context }) => 
    context.recording !== null,

  // Audio blob exists
  hasAudio: ({ context }) => 
    context.recording?.audioBlob !== undefined,

  // Should pause on user typing
  shouldPauseOnInteraction: ({ context }) => 
    context.pauseOnUserInteraction,
