...
- The current app emits schema version `4` recordings in SCR3 format v5. Ordinary local Monaco changes carry exact edit batches with base/result integrity checks. Append-only agent snapshots emit a checked equal/insert delta containing only the suffix without running Myers; provider rewrites, bulk/imported/remote editor changes, and preview changes retain the verified DMP fallback. Raw workspace assets use dedicated v4 segments. Format v5 records runtime terminal output as deltas between sparse full-snapshot checkpoints (`src/core/src/runtimeTrack.ts`; resolve an index with `resolveRuntimeSnapshotAt`), seeds the workspace-event content dedup from the header's `workspaceSnapshot`, and stores a changed file as a prefix/suffix splice when that is smaller. SCR3 formats v2–v4 remain readable.
