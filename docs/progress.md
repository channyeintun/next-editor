...
**Real bug found by Chan's first real production upload ("Effective workflow with coding agents", 28:08, published): the lesson page loaded but nothing played.** Root cause: `ne`/`thumbnail` were being stored in D1 as the bare R2 key (`lessons/<id>/<id>.ne`, matching what `POST /api/uploads/.../media/...` returns), but that key is only actually servable at `/media/<key>` (`infra/worker/routes/media.ts`) — the P1.1-era design comment ("stored WITHOUT a leading slash — tube's resolveThumb()/LessonDetail already prepend one, matching the static seed manifest's convention") was correct for the _seed_ (whose files really do live at the site root as static assets) but wrong for R2-backed uploads, which only exist under `/media/*`. `/${lesson.ne}` was resolving to `/lessons/<id>/<id>.ne` — no matching route, falls through to the SPA catch-all, returns `index.html` (200, `text/html`) instead of the recording — confirmed directly by curling both paths before fixing anything.

Fixed in `infra/worker/routes/lessons.ts` with a `toMediaPath()` helper applied when writing `ne`/`thumbnail` into D1 (`POST /api/lessons`, `PATCH /api/lessons/:id`) — the server, not the client, now owns the "raw upload key → public media path" mapping. Verified against local D1/R2 (upload → create → publish → confirm `/${lesson.ne}` now resolves to real bytes) before touching production. Only one lesson existed in the real database; corrected its `ne` value directly (`UPDATE lessons SET ne = 'media/' || ne WHERE ...`) rather than needing a general migration script. Redeployed; confirmed the real lesson's `.ne` (2,035,633 bytes) now serves correctly through the exact path the player uses.
