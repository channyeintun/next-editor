...
- **Status:** M0–M3 implemented (2026-07-21) in `src/studio/` — see [studio-m0-runbook.md](./studio-m0-runbook.md). M1: YAML `LessonScript`, marker compiler, TTS cache, Director CLI. M2: slide/whiteboard surfaces, silent runtime retry, unattended render command (go-cube-tour passed 2× headless: 12/12 QA, 7/7 repeatability, retry survived). M3: compiled timing gate + draft creation through the standard upload flow with provenance disclosure (publish stays a separate human action)
