...
const IdleRecordButton = memo(function IdleRecordButton() {
  const corgiRef = useRef<SVGSVGElement | null>(null);
  ...
  useEffect(() => {
    const corgi = corgiRef.current;
    const ears = earsRef.current;
    const tail = tailRef.current;
    const leftEye = leftEyeRef.current;
    const rightEye = rightEyeRef.current;

    if (!corgi || !ears || !tail || !leftEye || !rightEye) {
      return;
    }

    const peekAnimation = animate(corgi, {
      translateX: [-9, -2, -3, -9],
      translateY: [6, -1, 0, 6],
      rotate: ['-9deg', '2deg', '0deg', '-9deg'],
    ...
    const earAnimation = animate(ears, {
      translateY: [0, -0.6, 0],
      scaleY: [1, 1.04, 1],
    ...
    const tailAnimation = animate(tail, {
    ...
    const blinkAnimation = animate([leftEye, rightEye], {
      scaleY: [1, 0.62, 1],
      duration: 140,
      delay: 1000,
      loopDelay: 1900,
