...
function KitePlaygroundRunnerPanel() {
  ...
  useEffect(() => {
    // `.kite` files are highlighted with Monaco's `rust` grammar (see
    // languageForPath) — there is no Kite grammar, and Rust is much the closest
    // fit. Only one runner panel is ever mounted, so registering the same
    // language id here and in the Rust panel never collides.
