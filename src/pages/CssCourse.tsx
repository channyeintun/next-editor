                ...
                <div>
                  {group.lessons.map((lesson) => (
                    <div
                      ...
                      className={`pl-8 pr-4 py-3 border-b border-slate-700 cursor-pointer hover:bg-slate-800 focus:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-inset text-slate-100 transition-all duration-200 group ${currentLessonId === lesson.id ? 'bg-gradient-to-r from-indigo-900 to-indigo-800 hover:from-indigo-800 hover:to-indigo-700 border-l-4 border-l-indigo-400' : ''
                        }`}
                      tabIndex={0}
                      role="button"
                      onKeyDown={(e) => {
...
const CssCourse: React.FC = () => {
  const [recordMode] = React.useState<boolean>(() => sessionStorage.getItem('recordMode') === 'true');
