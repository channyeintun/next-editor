...
describe("runtime tokens", () => {
  it("round-trips signed claims", async () => {
    const token = await signToken("a sufficiently long test secret", { userId: "user-1", exp: Date.now()/1000+60, sessionId: "sid" });
    await expect(verifyToken("a sufficiently long test secret", token)).resolves.toMatchObject({ userId: "user-1", sessionId: "sid" });
  });
