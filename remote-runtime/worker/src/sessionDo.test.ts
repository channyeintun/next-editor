...
describe("session lifecycle accounting", () => {
  ...
  it("starts the agent and passes the configured workspace before provisioning succeeds", async () => {
    ...
    const response = await session.fetch(new Request("http://do/__runtime/configure", {
      method: "PUT",
      body: JSON.stringify({
        sessionId: "sid",
        userId: "user",
  ...
  it("fails provisioning and destroys the container when the health check fails", async () => {
    ...
    const session = new RuntimeGoSessionDO({ storage } as never, {
      RUNTIME_QUOTAS: {},
    } as unknown as Env);
  ...
  it("restores persisted idle and workspace settings before an alarm runs", async () => {
    const values = new Map<string, unknown>([["sessionConfig", {
      sessionId: "sid",
      userId: "user",
      createdAt: Date.now(),
