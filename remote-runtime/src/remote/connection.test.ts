...
class MockWebSocket extends EventTarget {
  ...
  send(data: string | ArrayBufferLike): void {
    ...
    const result =
      frame.m === "session.hello"
        ? {
            workdir: "/workspace/project",
            agentVersion: "test",
            resumed: MockWebSocket.instances.length > 1,
...
describe("RCP connection", () => {
  ...
  it("reconnects with the resume token and invokes re-registration hooks", async () => {
    ...
    const hello = MockWebSocket.instances[1]!.sent.find(
      (item) => typeof item === "string" && JSON.parse(item).m === "session.hello",
    );
  ...
  it("rejects oversized outbound control frames before registering a request", async () => {
    ...
    await connection.open();
    expect(() =>
      connection.beginRequest("proc.spawn", {
        cmd: "tool",
        args: [],
