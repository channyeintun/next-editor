...
describe("preview compatibility", () => {
  ...
  it("validates the origin rendered for the message's actual preview port", () => {
    ...
    expect(matchesPreviewOrigin(template, "abc", 8080, "https://p8080-abc.preview.test"))
      .toBe(true);
    expect(matchesPreviewOrigin(template, "abc", 8080, "https://p8600-abc.preview.test"))
      .toBe(false);
    expect(matchesPreviewMessageSource(
      template,
      "abc",
