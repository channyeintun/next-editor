...
export function matchesPreviewMessageSource(
  ...
  return (
    (!sessionId || message.previewId === sessionId) &&
    matchesPreviewOrigin(template, sessionId, message.port, origin)
  );
}

export class RemoteContainer implements Pick<
  WebContainer,
...
> {
  ...
  static async boot(options: RemoteBootOptions = {}): Promise<RemoteContainer> {
    ...
    try {
      ...
      const container = new RemoteContainer(
        connection,
        provisioned.previewUrlTemplate,
        {
          endpoint,
    ...
    const bytes =
      snapshotOrTree instanceof Uint8Array
        ...
        : snapshotOrTree instanceof ArrayBuffer
          ...
          : treeToZip(snapshotOrTree);
    const ch = this.connection.channels.allocate();
    const { response } = this.connection.beginRequest("mount", {
  ...
  async spawn(
    ...
    const env = Object.fromEntries(
      Object.entries(actualOptions.env ?? {}).map(([key, value]) => [key, String(value)]),
    );
    const result = await this.connection.request("proc.spawn", {
      ...
      terminal: actualOptions.terminal,
      output: actualOptions.output ?? true,
    });
  ...
  async setPreviewScript(src: string, options: PreviewScriptOptions = {}): Promise<void> {
    if (!this.control) {
      console.warn("setPreviewScript is unavailable in direct attach mode");
      return;
    }
    const editorOrigin = typeof window !== "undefined" ? window.location.origin : undefined;
    const prelude =
      this.forwardPreviewErrors && editorOrigin
        ? createPreviewErrorForwarder({
    ...
    const response = await fetch(
      `${this.control.endpoint}/sessions/${this.control.sessionId}/preview-script`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          ...(this.control.token ? { Authorization: `Bearer ${this.control.token}` } : {}),
        },
  ...
  teardown(): void {
    ...
    if (this.control)
      void fetch(`${this.control.endpoint}/sessions/${this.control.sessionId}`, {
        method: "DELETE",
        keepalive: true,
...
const _runtimeCompatibility: Pick<
  WebContainer,
  "fs" | "mount" | "spawn" | "export" | "on" | "setPreviewScript" | "teardown" | "path" | "workdir"
