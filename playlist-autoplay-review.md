...
6. **FIXED** — `setRecording` carries `timeline.speed` and `timeline.volume` across loads (the audio child spawns from these context values, so they reach it). Follow-up landed: speed/volume are now player-level settings in `playbackSettingsStore` (localStorage-persisted, clamped to the slider ranges); MediaControls writes user changes to both the machine and the store, and Editor hydrates a fresh machine instance from the store once a recording enters playback — so the setting survives remounts and full page loads, not just in-instance auto-advance.
